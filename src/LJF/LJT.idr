...
--    W   : Async g a -> Async (b::g) a
--    P   : Async (a::b::g) c -> Async (b::a::g) c
...
--    WC  : LSync g a b -> LSync (c::g) a b

-- TODO somehow totality checker takes a few minutes here without asserts
mutual 
...
reduceA : Async g b -> Maybe (Async g b)
reduceA (HC (IR t)     (IL u k)          ) = Just $ HC (MC u t) k
reduceA (HC (Foc el k)  m                ) = Just $ Foc el (FHC k m)
reduceA (MC  u         (IR t)            ) = Just $ IR $ MC (shiftAsync {is=ConsR Id} u) (shiftAsync t) --$ MC (W u) (P t)
